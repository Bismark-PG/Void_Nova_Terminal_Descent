...
bool Shader_Initialize(ID3D11Device* pDevice, ID3D11DeviceContext* pContext)
{
	...
	if (!pDevice || !pContext) {
		Debug::D_Out << "Shader_Initialize() : 与えられたデバイスかコンテキストが不正です" << std::endl;
		return false;
	}
	...
	if (FAILED(hr)) {
		Debug::D_Out << "Shader_Initialize() : 頂点シェーダーの作成に失敗しました" << std::endl;
	...
	if (FAILED(hr)) {
		Debug::D_Out << "Shader_Initialize() : 頂点レイアウトの作成に失敗しました" << std::endl;
		return false;
	}
	...
	if (FAILED(hr)) {
		Debug::D_Out << "Shader_Initialize() : ピクセルシェーダーの作成に失敗しました" << std::endl;
		return false;
	}
