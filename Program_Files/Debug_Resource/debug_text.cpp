...
namespace Text
{
	...
	void DebugText::Draw()
	{
		...
		const float characterWidth = (m_TextureWidth / 16.0f) * m_Scale;
		const float characterHeight = (m_TextureHeight / 16.0f) * m_Scale;

		// Use For Scale Set
		const float scaledLineSpacing = m_LineSpacing * m_Scale;
		const float scaledCharacterSpacing = m_CharacterSpacing * m_Scale;
			...
			for (const auto& string : strings.strings) {
				...
				for (const auto& code : string.characters) {
					...
					if (index) {
						...
						float x = m_OffsetX + columnCount * scaledCharacterSpacing;
						float y = m_OffsetY + lineCount * scaledLineSpacing;
	...
	// Release When Destory Window
	void DebugText::ReleaseAllTextures()
	{
		for (auto& pair : m_TextureMap)
		{
			std::get<0>(pair.second)->Release();
			std::get<1>(pair.second)->Release();
		}
